...
class StdStringBuilder {
 public:
  ...
  void Append(char ch) { buffer_.push_back(ch); }
  ...
  void AppendNumber(int num) { Append(std::to_string(num)); }

  void Clear() { buffer_.resize(0); }

  void ShrinkToFit() { buffer_.shrink_to_fit(); }

  void Reserve(intptr_t capacity) { buffer_.reserve(capacity); }

  std::string ToString() { return std::string(buffer_.data(), buffer_.size()); }

  const char* data() const { return buffer_.data(); }

  char* data() { return buffer_.data(); }

  intptr_t size() const { return buffer_.size(); }

 private:
  std::vector<char> buffer_;

  FTL_DISALLOW_COPY_AND_ASSIGN(StdStringBuilder);
};
...
Dart_Handle DartExceptionFactory::CreateException(const std::string& class_name,
  ...
  Dart_Handle exception_class =
      Dart_GetType(core_library_.value(), StdStringToDart(class_name), 0, 0);
