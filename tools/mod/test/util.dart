...
String getTestScriptPath() {
  ...
  if (!scriptPath.endsWith('_test.dart')) {
    ...
    if (Platform.script.scheme == 'data') {
      String encodedData =
          Platform.script.path.substring(Platform.script.path.indexOf(',') + 1);
      scriptContent = Uri.decodeComponent(encodedData);
    } else {
      scriptContent = new File.fromUri(Platform.script).readAsStringSync();
    }

    RegExp pattern =
        new RegExp(r'''import ['"]file://(.+_test.dart)['"] as test;''');
